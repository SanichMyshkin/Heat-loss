...
class GUI(QMainWindow, MaterialsTable,
    ...
    def TutorialMessage(self):
        ...
        message_box.setFixedSize(700, 900)
        ...
        K - Коэффициент теплопроводности, Вт/(м2 ˚С)
        R - Сопротивление теплопередачи, м² (°C/Вт)
        Q - Теплопотери, кВт/ч

        Нормативные документы:
            1.	Справочник проектировщика. Внутренние санитарно-технические устройства, часть 1. Отопление. 1975 г. Раздел 8.2
            2.	Лабораторная работа: Автоматизации расчета тепловых потерь помещений

        Процесс расчета:
            Резульатотм вычисления является подсчет теплопетерь между помещениями по формуле Q = kA(t1 - t2)
            Коэффициент k рассчитывается по формуле k = 1/R
    ...
    def AuthorsMessage(self):
        message_box = QMessageBox()
        message_box.setFixedSize(600, 400)
        message_box.information(
            None, "Авторы",
            '''Данное приложение было разработано:
            Cтудентами НИУ МГСУ, 4 курса направления ИСТАС, группы ИЦТМС 4-2
            Мышкиным А.В.\nи\nДубровиным В.А.
            \t2023 г.''')
